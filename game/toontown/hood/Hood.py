...
from toontown.dna import DNAParserOld
...
class Hood(StateData.StateData):
    ...
    def load(self):
        ...
        if self.storageDNAFile:
            if self.storageDNAFile[-4:] == '.dna':
                DNAParserOld.loadDNAFileOld(self.dnaStore, self.storageDNAFile)
            else:
        ...
        if not newsManager:
            ...
            self.sky.setScale(1.0)
            self.sky.setFogOff()
        if self.storageDNAFile is None:
            dnaBulk = DNABulkLoader(self.dnaStore, tuple(files))
            dnaBulk.loadDNAFiles()
        elif self.storageDNAFile[-4:] != '.dna':
            dnaBulk = DNABulkLoader(self.dnaStore, tuple(files))
            dnaBulk.loadDNAFiles()
