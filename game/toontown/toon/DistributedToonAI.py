...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def b_setAnimState(self, animName, animMultiplier, timestamp = 0):
        self.setAnimState(animName, animMultiplier, timestamp)
        self.d_setAnimState(animName, animMultiplier, timestamp)

    def d_setAnimState(self, animName, animMultiplier, timestamp = 0):
    ...
    def b_setAnimPlay(self, animName, type, startFrame=0, endFrame=0, timestamp = 0):
        self.setAnimPlay(animName, type, startFrame, endFrame, timestamp)
        self.d_setAnimPlay(animName, type, startFrame, endFrame, timestamp)

    def d_setAnimPlay(self, animName, type, startFrame=0, endFrame=0, timestamp = 0):
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setAnimPlay', [animName, type, startFrame, endFrame, timestamp])
        return None

    def setAnimPlay(self, animName, type, startFrame=0, endFrame=0, timestamp = 0):
        self.animName = animName
        self.type = type
    ...
    def b_setCogIndex(self, index, type=0):
        self.setCogIndex(index)
        if simbase.config.GetBool('cogsuit-hack-prevent', False):
            self.d_setCogIndex(self.cogIndex, type)
        else:
            self.d_setCogIndex(index, type)

    def setCogIndex(self, index, type=0):
        self.cogIndex = index

    def d_setCogIndex(self, index, type=0):
        self.sendUpdate('setCogIndex', [index, type])
    
    def b_charTrans(self, index):
        self.charTrans(index)
        self.d_charTrans(index)
    ...
    def doGreen(self, type, cogType='f', toonId=20001):
        self.sendUpdate('doGreen', [type, cogType, toonId])
        if type == 'cog':
            Sequence(Wait(11), Func(self.takeDamage, self.maxHp)).start()
        elif type == 'toon':
            Sequence(Wait(5.75), Func(self.takeDamage, self.maxHp)).start()
        else:
            self.takeDamage(self.maxHp)

@magicWord(category=CATEGORY_MODERATOR, types=[str, int])
def setAnimState(anim, speed=1):
    """Change the target's animstate."""
    invoker = spellbook.getInvoker()
    invoker.b_setAnimState(anim, speed)
    return 'Set your animState to %s at speed %d.' % (anim, speed)
...
def pose(anim, frame):
    ...
    """
    target = spellbook.getTarget()
    target.b_setAnimPlay(anim, 2, frame)
    return 'Posing animation %s at frame %d.' % (anim, frame)


@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[str, int, int])
def pingpong(anim, start, end):
    """
    ...
    """
    target = spellbook.getTarget()
    target.b_setAnimPlay(anim, 1, start, end)
    return 'Looping animation %s from frame %d to frame %d, back and forth...' % (anim, start, end)
...
def maxToon(confirm=None, missingTrack=None):
    """
    ...
    if confirm != 'CONFIRM':
        return 'Are you suuuuuuuuure?  If you are...\n~maxToon CONFIRM'
...
@magicWord(category=CATEGORY_PROGRAMMER)
def emotes():
    """Unlocks the invoker's emotions."""
    ...
    return 'Unlocked basic emotes!  Contact the server host for more emotes.'
...
@magicWord(category=CATEGORY_CREATIVE, types=[int, int])
def setGlasses(glassesIndex, glassesTex=0):
    """
...
@magicWord(category=CATEGORY_CREATIVE, types=[int, int])
def setBackpack(backpackIndex, backpackTex=0):
    """
...
@magicWord(category=CATEGORY_CREATIVE, types=[int, int])
def setShoes(shoesIndex, shoesTex=0):
    """
...
def setCogIndex(index, type=0):
    """
    Modifies the invoker's Cog index.
    """
    if not -1 <= index <= 4:
        return 'Invalid Cog index. (-1-4)'
    if not 0 <= type <= 4:
        return 'Invalid Cog type. (0-4)'
    invoker = spellbook.getInvoker()
    if invoker.charIndex != -1:
        return 'Can\'t disguise if you\'re transformed!'
    invoker.b_setCogIndex(index, type)
    if index != -1:
        return 'Set your Cog index to %d with type %d!' % (index, type)
    else:
        return 'Removed your Cog suit!'
        
@magicWord(category=CATEGORY_ADMINISTRATOR, types=[int])
def charTrans(index, preview=0):
    """
    Transforms invoker into a classic character.
    """
    from toontown.char import CharDNA
    if preview == 1:
        return 'CharID = %s' % (charTypes[index])
    if not -1 <= index <= 17:
        return 'Invalid index. (-1-17)'
    invoker = spellbook.getInvoker()
    if invoker.cogIndex != -1:
        return 'Can\'t turn into a ClassicChar if you\'re disguised!'
    invoker.b_charTrans(index)
    if index != -1:
...
def inventory(a, b=None, c=None):
    """
    Modifies gags in invoker's inventory.
    """
    invoker = spellbook.getInvoker()
...
def dna(part, value):
    ...
    if part == 'torso':
        ...
        value = int(value)
        if (not 0 <= value <= 8):
            return 'Torso index out of range (0-8).\n0-2 = shorts\n3-5 = skirts\n6-8 = nude\nLowest=Smallest Body'
...
@magicWord(category=CATEGORY_PROGRAMMER, types=[str, int, str])
def setCogSuit(suitName, level, value):
    invoker = spellbook.getInvoker()
    
    if suitName == 'bossbot':
        suitIndex = 0
    elif suitName == 'lawbot':
        suitIndex = 1
    elif suitName == 'cashbot':
        suitIndex = 2
    elif suitName == 'sellbot':
        suitIndex = 3
    elif suitName == 'crazybot':
        suitIndex = 4
    else:
        return 'Invalid cog type: %s\nMake sure it\'s lowercase.' % suitName
        
    if value not in SuitDNA.suitHeadTypes:
        return 'Invalid cog name: %s' % value
    
    invoker.cogTypes[suitIndex] = SuitDNA.suitHeadTypes.index(value) - (9*suitIndex)
    invoker.d_setCogTypes(invoker.cogTypes)
    invoker.cogLevels[suitIndex] = level
    invoker.d_setCogLevels(invoker.cogLevels)
    return 'Set cog suit!'

@magicWord(category=CATEGORY_PROGRAMMER, types=[int, int])
def merits(suitIndex, value):
    invoker = spellbook.getInvoker()

    if not 0 <= suitIndex <= 3:
        return 'Invalid Cog index. (0-3)'
    if not 0 <= value <= 23300:
        return 'Invalid merits. (0-23300)'
...
def leech(laff):
    """
    Leech another toon.  Made by TrueBlueDogemon
    """
    invoker = spellbook.getInvoker()
    target = spellbook.getTarget()
    if invoker == target:
        return 'You cannot leech off of yourself!'
    target.takeDamage(laff)
    invoker.toonUp(laff)
    return 'Leeched %d laff out of %s!' % (laff, target.getName())

@magicWord(category=CATEGORY_MODERATOR, types=[str, str])
def green(type, name=None):
    """
    Spawns either a Cog or a Toon to kill you.
    """
    target = spellbook.getTarget()
    ...
    if not name:
        if type == 'cog':
            ...
            name = TTLocalizer.Flippy
        else:
            return 'Invalid type. (toon or cog)'

    if name.lower() == 'HQ Officer'.lower() and type == 'toon':
        return 'You cannot be an HQ Officer!'

    if type == 'cog':
        if name not in SuitDNA.suitHeadTypes:
            return 'Invalid Cog name: %s' % name
        else:
            target.doGreen(type=type, cogType=name)
            #taskMgr.doMethodLater(8, target.setHp, 'hurtGreen-Cog', extraArgs=[-1])
            if target == spellbook.getInvoker():
                return 'Greened yourself with a Cog!'
            else:
                return 'Greened %s with a Cog!' % target.getName()
    elif type == 'toon':
        for npcId, npcName in TTLocalizer.NPCToonNames.items():
            if name.lower() == npcName.lower():
                id = npcId
                print ':DistributedToonAI: %s Green ID + Name == %d, %s' % (spellbook.getInvoker().getName(), id, name)
                #taskMgr.doMethodLater(5, target.setHp, 'hurtGreen-Toon', extraArgs=[-1])
                target.doGreen(type=type, toonId=id)
                if target == spellbook.getInvoker():
                    return 'Greened yourself with a Toon!'
                else:
                    return 'Greened %s with a Toon!' % target.getName()
        else:
            return 'Invalid Toon name: %s' % name
    else:
        return 'Invalid type. (toon or cog)'
...
def immortal():
    ...
    """
    invoker = spellbook.getInvoker()
    if invoker.immortalMode == 0:
        invoker.setImmortalMode(1)
        return "Enabled Immortal Mode."
    else:
        invoker.setImmortalMode(0)
        return "Disabled Immortal Mode."