...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def b_setAnimPlay(self, animName, type, startFrame=0, endFrame=0):
        self.setAnimPlay(animName, type, startFrame, endFrame)
        self.d_setAnimPlay(animName, type, startFrame, endFrame)

    def d_setAnimPlay(self, animName, type, startFrame=0, endFrame=0):
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setAnimPlay', [animName, type, startFrame, endFrame, timestamp])

    def setAnimPlay(self, animName, type, startFrame=0, endFrame=0, timestamp = None):
        if timestamp == None:
            ts = 0.0
        else:
            ts = globalClockDelta.localElapsedTime(timestamp)
        #callback = None
        if type == 0:
            if self.isDisguised != 1:
                self.loop(animName)
            elif self.isTransformed == 1:
                self.char.loop(animName)
            else:
                self.suit.loop(animName)
        elif type == 1:
            if self.isDisguised != 1:
                self.pingpong(animName, startFrame, endFrame)
            elif self.isTransformed == 1:
                self.char.pingpong(animName, startFrame, endFrame)
            else:
                self.suit.pingpong(animName, startFrame, endFrame)
        elif type == 20: #flower
            if self == base.localAvatar:
                if hasattr(self, 'book'):
                    self.book.obscureButton(1)
                base.localAvatar.disableAvatarControls()
                Sequence(Wait(0.2), Func(self.play, animName), Wait(0.7), Func(base.playSfx, base.loadSfx('phase_4/audio/sfx/MG_cannon_hit_dirt.ogg'), volume=1, node=self), Wait(self.getDuration(animName) - 0.7), Func(self.loop, 'neutral')).start()
            else:
                self.play(animName)
        else:
            if self.isDisguised != 1:
                self.pose(animName, startFrame)
            elif self.isTransformed == 1:
                self.char.pose(animName, startFrame)
            else:
                self.suit.pose(animName, startFrame)
        return
    ...
    def setCogIndex(self, index, type=0):
        self.cogIndex = index
        #print 'CogIndexType = %d' % type
        if self.cogIndex == -1:
            if self.isDisguised:
                self.takeOffSuit()
        else:
            ...
            if CogDisguiseGlobals.isPaidSuitComplete(self, parts, index):
                ...
                if type == 1:
                    self.putOnSuit(cog, 1)
                elif type == 2:
                    self.putOnSuit(cog, 0, True)
                elif type == 3:
                    self.putOnSuit(cog, 1, True)
                elif type == 4:
                    self.putOnSuit(cog, 2)
                else:
                    self.putOnSuit(cog)
            else:
                if type == 1:
                    self.putOnSuit(index, 1, rental=True)
                elif type == 2:
                    self.putOnSuit(index, 0, True, rental=True)
                elif type == 3:
                    self.putOnSuit(index, 1, True, rental=True)
                elif type == 4:
                    self.putOnSuit(index, 2, rental=True)
                else:
    ...
    def loadSummon(self, cogIndex):
        self.summonDialog = SummonCogDialog.SummonCogDialog(cogIndex)
        self.summonDialog.load()
        self.summonDialog.enter()

    def doGreen(self, type, cogType, toonId):
        spawnPos = self.getPos()
        spawnHpr = self.getHpr()
        if type == 'cog':
            skyPos = Point3(spawnPos)
            skyPos.setZ(spawnPos.getZ() + SuitTimings.fromSky * ToontownGlobals.SuitWalkSpeed)
            cog = Suit.Suit()
            ...
            cog.setDNA(cogDNA)
            cog.addActive()
            cog.initializeBodyCollisions('suit')
            cog.cogProp = BattleProps.globalPropPool.getProp('propeller')
            if cog.style.body == 'a':
                head = cog.find('**/to_head')
            else:
                head = cog.find('**/joint_head')
            cog.cogProp.reparentTo(head)
            propInSound = base.loadSfx('phase_5/audio/sfx/ENC_propeller_in.ogg')
            propOutSound = base.loadSfx('phase_5/audio/sfx/ENC_propeller_out.ogg')
            speakSound = base.loadSfx('phase_3.5/audio/dial/COG_VO_statement.ogg')
            groundF = 28
            ...
            propDur = cog.cogProp.getDuration('propeller')
            propFr = cog.cogProp.getFrameRate('propeller')
            lastSpinFrame = 8
            spinTime = lastSpinFrame / propFr
            openTime = (lastSpinFrame + 1) / propFr
            # ---time to make a new toon >:(
            timeToMake = Sequence(Func(cog.setChatAbsolute, 'Time to make a new Toon!', CFSpeech | CFTimeout),
                                  SoundInterval(speakSound, duration=waitTime + dur, node=cog),
                                  ActorInterval(cog, 'victory'))
            # ---EnterAnims
            enterlerpPosTrack = Sequence(cog.posInterval(timeTillLanding, spawnPos, startPos=skyPos))
            entershadowScale = cog.dropShadow.getScale()
            entershadowTrack = Sequence(Func(cog.dropShadow.reparentTo, render), Func(cog.dropShadow.setPos, spawnPos),
                                        cog.dropShadow.scaleInterval(timeTillLanding, cog.scale,
                                                                     startScale=Vec3(0.01, 0.01, 1.0)),
                                        Func(cog.dropShadow.reparentTo, cog.getShadowJoint()),
                                        Func(cog.dropShadow.setPos, 0, 0, 0),
                                        Func(cog.dropShadow.setScale, entershadowScale))
            enterfadeInTrack = Sequence(Func(cog.setTransparency, 1),
                                        cog.colorScaleInterval(1, colorScale=VBase4(1, 1, 1, 1),
                                                               startColorScale=VBase4(1, 1, 1, 0)),
                                        Func(cog.clearColorScale), Func(cog.clearTransparency))
            enteranimTrack = Sequence(Func(cog.pose, 'landing', 0), Wait(waitTime), ActorInterval(cog, 'landing'),
                                      Parallel(Sequence(
                                          Func(cog.setChatAbsolute, 'Time to make a new Toon!', CFSpeech | CFTimeout),
                                          SoundInterval(speakSound, node=cog),
                                          ActorInterval(cog, 'victory')), Sequence(Wait(2))))
            enterpropTrack = Parallel(SoundInterval(propInSound, duration=waitTime + dur, node=cog), Sequence(
                ActorInterval(cog.cogProp, 'propeller', constrainedLoop=1, duration=waitTime + spinTime, startTime=0.0,
                              endTime=spinTime),
                ActorInterval(cog.cogProp, 'propeller', duration=propDur - openTime, startTime=openTime),
                Func(cog.cogProp.hide)))
            enterTrack = Parallel(enterlerpPosTrack, entershadowTrack, enterfadeInTrack, enteranimTrack, enterpropTrack)
            # ---ExitAnims
            exitlerpPosTrack = Sequence(Wait(impactLength),
                                        LerpPosInterval(cog, timeTillLanding, skyPos, startPos=spawnPos))
            exitshadowTrack = Sequence(Func(cog.dropShadow.reparentTo, render), Func(cog.dropShadow.setPos, spawnPos),
                                       cog.dropShadow.scaleInterval(timeTillLanding, Vec3(0.01, 0.01, 1.0),
                                                                    startScale=cog.scale),
                                       Func(cog.dropShadow.reparentTo, cog.getShadowJoint()),
                                       Func(cog.dropShadow.setPos, 0, 0, 0))
            exitfadeOutTrack = Sequence(Func(cog.setTransparency, 1),
                                        cog.colorScaleInterval(1, colorScale=VBase4(1, 1, 1, 0),
                                                               startColorScale=VBase4(1, 1, 1, 1)),
                                        Func(cog.clearColorScale), Func(cog.clearTransparency),
                                        Func(cog.reparentTo, hidden))
            exitactInt = ActorInterval(cog, 'landing', loop=0, startTime=dur, endTime=0.0)
            exitpropTrack = Parallel(SoundInterval(propOutSound, duration=waitTime + dur, node=cog),
                                     Sequence(Func(cog.cogProp.show),
                                              ActorInterval(cog.cogProp, 'propeller', endTime=openTime,
                                                            startTime=propDur),
                                              ActorInterval(cog.cogProp, 'propeller', constrainedLoop=1,
                                                            duration=propDur - openTime, startTime=spinTime,
                                                            endTime=0.0), Func(cog.cogProp.hide)))
            exitTrack = Parallel(ParallelEndTogether(exitlerpPosTrack, exitshadowTrack, exitfadeOutTrack), exitactInt,
                                 exitpropTrack)
            ...
            fullTrack = Sequence(enterTrack, exitTrack, Func(cog.cogProp.cleanup), Func(cog.cogProp.removeNode),
                                 Func(cog.cogProp.delete),
                                 Func(cog.removeActive), Func(cog.cleanup), Func(cog.removeNode), Func(cog.delete))
            ...
            for npcId, npcInfo in NPCToons.NPCToonDict.items():
                if npcId == toonId:
                    dnaList = npcInfo[2]
                    break
            toonDNA.newToonFromProperties(*dnaList)
            toon.setDNA(toonDNA)
            toon.setName(TTLocalizer.NPCToonNames[toonId])
            toon.setPickable(0)
            toon.addActive()
            toon.initializeBodyCollisions('toon')
            toon.setPlayerType(NametagGlobals.CCNonPlayer)
            enterTrack = Sequence(Func(toon.animFSM.request, 'TeleportIn'), Wait(1.517),
                                  Func(toon.animFSM.request, 'neutral'))
            timeToMake = Sequence(Func(toon.lookAt, self), Func(toon.setP, 0), Func(toon.setR, 0),
                                  Func(toon.setChatAbsolute, 'You stink!', CFSpeech | CFTimeout),
                                  Func(toon.doNPCEmote, 3), Wait(3),
                                  Parallel(Sequence(
                                      Func(toon.setChatAbsolute, 'Time to make a new Toon!', CFSpeech | CFTimeout),
                                      ActorInterval(toon, 'hypnotize')),
                                      Sequence(Wait(0.5))))
            exitTrack = Sequence(Func(toon.animFSM.request, 'TeleportOut'),
                                 Wait(toon.getDuration('teleport')))
            toon.setPos(spawnPos)
            toon.setHpr(spawnHpr)
            toon.reparentTo(render)
            fullTrack = Sequence(enterTrack, timeToMake, exitTrack, Func(toon.stopBlink), Func(toon.removeActive),
                                 Func(toon.cleanup), Func(toon.removeNode), Func(toon.delete))
...
def summons(cogIndex):
    """
    Loads the Cog Summon Dialog Box for the Cog specified
    """
    invoker = spellbook.getInvoker()
    
    if cogIndex not in SuitDNA.suitHeadTypes:
        return 'Invalid cog index: %s' % cogIndex
    newCogIndex = SuitDNA.suitHeadTypes.index(cogIndex)
    print newCogIndex
    invoker.loadSummon(newCogIndex)
    return 'Magic word executed successfully!'
    
@magicWord(category=CATEGORY_USER)
def sleep():
    """
    Prevents the user from sleeping.
    """
    invoker = spellbook.getInvoker()
    
    if invoker.noSleep == 0:
        invoker.noSleep = 1
        return 'Sleeping has been disabled for the current session.'
    else:
        invoker.noSleep = 0
        return 'Sleeping has been reenabled.'

'''@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[str])
def loop(anim):
    """Loops an animation on a target"""
    invoker = spellbook.getInvoker()
    invoker.b_setAnimPlay(anim, 0)
    return 'Looping animation %s.' % anim

@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[str, int, int])
def pingpong(anim, startFrame, endFrame):
    """Loops an animation on a target"""
    invoker = spellbook.getInvoker()
    invoker.b_setAnimPlay(anim, 1, startFrame, endFrame)
    return 'Looping animation %s from frame %d to frame %d.' % (anim, startFrame, endFrame)

@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[str, int])
def pose(anim, startFrame):
    """Loops an animation on a target"""
    invoker = spellbook.getInvoker()
    invoker.b_setAnimPlay(anim, 2, startFrame)
    return 'Posing animation %s on frame %d.' % (anim, startFrame)'''