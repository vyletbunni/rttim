...
class DistributedElectionEvent(DistributedObject, FSM):
    ...
    def __init__(self, cr):
        ...
        if config.GetBool('want-doomsday', False):
            ...
            self.dimm.nametagLods = [] # Stolen from when Joey made the nametag follow the toon's head when they teleported.
    ...
    def __cleanupNPCs(self):
        ...
        if not config.GetBool('want-doomsday', False):
            npcs = [self.flippy, self.slappy, self.alec, self.surlee, self.surleeR, self.suit]
        for npc in npcs:
            if npc:
                npc.removeActive()
    ...
    def exitBegin(self):
        ...
        # This is still going on when Begin starts. Just cleaning it up here rather than pre-show.
    ...
    def enterCogLanding(self, offset):
        ...
        self.cogSequence = Sequence(
            ...
            Wait(4),
            # The Yesman has found himself a new business partner.
    ...
    def enterWrapUp(self, offset):
        ...
        # Let's make some sequences for the HUD to go away.
        ...
        #self.friendButton = base.localAvatar.bFriendsList
        base.localAvatar.setPieType(0) # Get rid of the Toon's pies.
        base.localAvatar.setNumPies(0)
        base.localAvatar.disableAvatarControls() # Prevent the avatar from grabbing more pies or opening a dialog.

        self.clearHudSeq = Sequence(
            Parallel(self.laffMeter.posInterval(10,(-0.5, 0.0, 0.13)), self.book.posInterval(10,(0.5, 0.0, 0.17)), self.book2.posInterval(10,(0.5, 0.0, 0.17)))
        )
        ...
        self.cameraSequence = Sequence(
            Func(base.cr.cameraManager.tvFlyOut.start),
            # Begin to slowly drift towards Flippy as he delivers his speech
