...
class DistributedInvasionSuit(DistributedSuitBase, InvasionSuitBase, FSM, DelayDeletable):
    ...
    def generateAnimDict(self):
        ...
        if self.style.name == 'ms':
            # Movers & Shakers should stomp instead of walk
    ...
    def __placeOnGround(self):
        # This schedules a task to fire after the shadow-culling to place the suit directly on his shadow.
    ...
    def setInvasionFinale(self, finale):
        if finale and not self.invasionFinale:
            ...
            self.setPickable(0) # We don't want people to see the cog's true identity, a Level 11 Loan Shark.
            self.setScale(1.1)
