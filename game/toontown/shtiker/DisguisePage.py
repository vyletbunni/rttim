...
class DisguisePage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        crazyButton = loader.loadModel('phase_3.5/models/gui/matching_game_gui.bam')
        ...
        for dept in SuitDNA.suitDepts:
            ...
            if dept != 'z':
                ...
                #pageGeom = gui.find('**/page1')
                tabGeom = crazyButton.find('**/minnieCircle')
                tab = DirectButton(
                    parent=self, relief=None, geom=tabGeom,
                    geom_color=DeptColors[4], pos=(0.65, 0, 0.6), scale=0.47,
                    text=SuitDNA.suitDeptFullnames[dept],
                    text_font=ToontownGlobals.getSuitFont(), text_scale=TTLocalizer.DPtab,
                    text_align=TextNode.ACenter, text1_fg=Vec4(1, 0, 0, 1),
                    text2_fg=Vec4(0.5, 0.4, 0.4, 1),
                    text3_fg=Vec4(0.4, 0.4, 0.4, 1), command=self.doTab,
                    extraArgs=[len(self.tabs)], pressEffect=0)
