...
from FlowerThump import FlowerThump
SPIN_RATE = 12.5
INTENSITY = 0
...
class DistributedDGFlower(DistributedObject.DistributedObject):
    ...
    def generate(self):
        ...
        #self.shake = ShakeCamera()
        self.thump = FlowerThump()
    ...
    def readyOrNot(self, intensity):
        print ':DistributedDGFlower: FOOL.'
        self.thump = FlowerThump()
        pos = self.bigFlower.getPos()
        self.bigFlower.posInterval(2, (pos[0], pos[1], 2.0)).start()
        self.thump.setPos(pos[0], pos[1], 1.5)
        self.thump.reparentTo(render)
        self.thump.start()
        INTENSITY = 10
        ShakeCamera(1).start()
