...
import math, random
from DNATypesetterOld import DNATypesetter
...
class DNABulkLoaderOld:
    ...
    def loadDNAFiles(self):
        for file in self.dnaFiles:
            ...
            loadDNABulkOld(self.dnaStorage, file)
        del self.dnaStorage
        del self.dnaFiles
...
def loadDNAFileOld(dnaStore, filename):
...
def loadDNABulkOld(dnaStorage, file):
    #dnaLoader = DNALoader()
    if __debug__:
        file = '../resources/' + file
    else:
...
def loadDNAFileOldAI(dnaStore, filename):
    dnaloader = DNALoader()
    dnaloader.getData().setDnaStorage(dnaStore)
    if __debug__:
        filename = '../resources/' + filename
...
def loadDNAFile4ToonFest(filename):
    dnaloader = DNALoader()
    if __debug__:
        filename = '../resources/' + filename
    else:
