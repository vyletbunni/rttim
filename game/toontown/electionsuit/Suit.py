...
class Suit(Avatar.Avatar):
    ...
    def generateSuit(self):
        ...
        if dna.name == 'f':
            ...
            self.scale = scale / aSize
            self.handColor = VBase4(0.7, 0.9, 0.5, 1.0)
            self.generateBody()
	    self.headTexture = 'the-big-cheese.jpg'
            ...
            self.scale = scale / aSize
            self.handColor = VBase4(0.50, 0.38, 0.60, 1.0)
            self.generateBody()
            self.generateHead('backstabber')
            self.setHeight(6.71)
        elif dna.name == 'sd':
            self.scale = scale / bSize
            self.handColor = VBase4(0.54, 0.84, 0.70, 1.0)
            self.generateBody()
            ...
            self.headColor = SuitDNA.moneyPolyColor
            self.generateBody()
            self.headTexture = 'short-change.jpg'
            ...
            self.scale = scale / bSize
            self.handColor = VBase4(0.6, 0.7, 0.65, 1.0)
            self.generateBody()
    ...
    def setSuitClothes(self, modelRoot = None):
        ...
        def __doItTheOldWay__():
            ...
            legTex.setMinfilter(Texture.FTLinearMipmapLinear)
            legTex.setMagfilter(Texture.FTLinear)
            handTex = loader.loadTexture('phase_3.5/maps/cog_hand.jpg')
    ...
    def makeWaiter(self, modelRoot = None):
        ...
        phase = 3.5
        dept = self.style.dept
        torsoTex = loader.loadTexture('phase_%s/maps/waiter_%s_blazer.jpg' % (phase, dept))
        torsoTex.setMinfilter(Texture.FTLinearMipmapLinear)
        torsoTex.setMagfilter(Texture.FTLinear)
        legTex = loader.loadTexture('phase_%s/maps/waiter_%s_leg.jpg' % (phase, dept))
        legTex.setMinfilter(Texture.FTLinearMipmapLinear)
        legTex.setMagfilter(Texture.FTLinear)
        armTex = loader.loadTexture('phase_%s/maps/waiter_%s_sleeve.jpg' % (phase, dept))
        armTex.setMinfilter(Texture.FTLinearMipmapLinear)
        armTex.setMagfilter(Texture.FTLinear)
        WaiterhandTex = loader.loadTexture('phase_3/maps/hand.jpg')
        modelRoot.find('**/torso').setTexture(torsoTex, 1)
        modelRoot.find('**/arms').setTexture(armTex, 1)
        modelRoot.find('**/legs').setTexture(legTex, 1)
        modelRoot.find('**/hands').setTexture(WaiterhandTex, 1)
        self.waiterHandColor = VBase4(0.95, 0.95, 1.0, 1.0)
        modelRoot.find('**/hands').setColor(self.waiterHandColor)
    ...
    def generateHead(self, headType):
        ...
        if headType == 'legaleagle':
            prop = loader.loadModel('phase_4/models/accessories/tt_m_chr_avt_acc_pac_angelWings')
            prop.setScale(5)
            prop.setPosHpr(0,0,0,0,0,0)
            prop.reparentTo(headModel)
