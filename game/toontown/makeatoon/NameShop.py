...
class NameShop(StateData.StateData):
    ...
    def __init__(self, parentFSM, doneEvent, avList, index, isPaid):
        ...
        self.fsm = ClassicFSM.ClassicFSM('NameShop', [State.State('Init', self.enterInit, self.exitInit, ['PayState']),
         ...
         State.State('ApprovalAccepted', self.enterApprovalAcceptedState, self.exitApprovalAcceptedState, ['TypeAName', 'Done']),
    ...
    def findTempName(self):
        animaltype = self.toon.style.getAnimal().capitalize()
        if animaltype == 'Fowl':
            animaltype = 'Duck'
    ...
    def enterAcceptedState(self):
        ...
        self.acceptedDialog = ToontownDialog.GlobalDialog(doneEvent = 'acceptedDone', message = TTLocalizer.NameShopNameAccepted, style = ToontownDialog.Acknowledge)
    ...
    def enterRejectedState(self):
        ...
        self.rejectedDialog = ToontownDialog.GlobalDialog(doneEvent = 'rejectedDone', message = TTLocalizer.NameShopNameRejected, style = ToontownDialog.Acknowledge)
    ...
    def __isFirstTime(self):
        if not self.nameList or self.newwarp:
            self.__createAvatar()
        else:
            self.promptTutorial()
