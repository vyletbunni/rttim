...
from toontown.toontowngui import MakeClothesGUI
import NameShop
...
class MakeAToon(StateData.StateData):
    ...
    def enter(self):
        ...
        base.playMusic(self.music, looping = 1, volume = 1.0)
        ...
        camera.setPosHpr(0.75, -17, 4.55, 0, -7.1200000000000001, 0)
        if self.warp:
    ...
    def load(self):
        ...
        self.guiTopBar.hide()
        self.guiBottomBar = DirectFrame(relief = None, image = gui.find('**/CrtATn_BtmBar'), image_scale = ((base.win.getXSize()/948.14814814814814814814814814815) * 0.98765432098765432098765432098765, 1, 1), pos = (0, 0, -0.85999999999999999))
        self.guiBottomBar.hide()
        self.guiCheckButton = DirectButton(parent = self.guiBottomBar, relief = None, image = (gui.find('**/CrtAtoon_Btn1_UP'), gui.find('**/CrtAtoon_Btn1_DOWN'), gui.find('**/CrtAtoon_Btn1_RLLVR')), pos = (base.a2dRight - 0.165, 0, -0.017999999999999999), command = self._MakeAToon__handleNext, text = ('', TTLocalizer.MakeAToonDone, TTLocalizer.MakeAToonDone), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.080000000000000002, text_pos = (0, -0.029999999999999999), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1))
    ...
    def unload(self):
        ...
        try:
            self.exitGenderTrack.stop()
            del self.exitGenderTrack
        except:
            print 'MakeAToon: No exitGenderTrack'
    ...
    def hostSez(self, stringList):
        self.host.setChatAbsolute(random.choice(stringList), CFSpeech)
        self.host.playDialogue('statementA', 1)
        return None

    
    def charSez(self, char, statement):
        char.setChatAbsolute(statement, CFSpeech)
        char.playDialogue('statementA', 1)
        return None
    ...
    def charRunFromLeft(self, char, newpos, newhpr, timelimit = 1.6000000000000001, comm = None, eA = []):
        myInterval = Sequence()
        myInterval.append(FunctionInterval(self.host.clearChat))
        myInterval.append(FunctionInterval(char.setPosHpr, extraArgs = [
            -12 + newpos[0],
            -4.5,
            0,
            270,
            ...
            0]))
        if not (self.slide):
            myInterval.append(FunctionInterval(char.loop, extraArgs = [
                'run']))
        ...
        myInterval.append(FunctionInterval(char.loop, extraArgs = [
            'neutral']))
        if comm:
            myInterval.append(FunctionInterval(comm, extraArgs = eA))
    ...
    def genderShopOpening(self, task):
        self.host = self.minnie
        self.hostSez([
            TTLocalizer.GenderShopQuestionMinnie])
        self.host = self.mickey
        self.hostSez([
            TTLocalizer.GenderShopQuestionMickey])
        return Task.done
    ...
    def _MakeAToon__handleGenderShopDone(self):
        ...
        gendershopInterval.append(WaitInterval(1.0))
        if self.toon.style.gender == 'm':
            self.timeToGetOff = 1
            self.minnie.clearChat()
            gendershopInterval.append(FunctionInterval(self.charSez, extraArgs = [
                self.minnie,
                TTLocalizer.GenderShopSeeYou]))
            self.host = self.mickey
            self.hostSez([
                TTLocalizer.GenderShopFollow])
        else:
        ...
        gendershopInterval.append(FunctionInterval(self.host.clearChat))
        gendershopInterval.append(self.charRunToRight(char = self.host, timelimit = self.timeToGetOff, comm = self.goToNextShop))
        gendershopInterval.append(FunctionInterval(self.mickey.clearChat))
        gendershopInterval.append(FunctionInterval(self.minnie.clearChat))
    ...
    def enterClothesShop(self):
        ...
        if len(self.dna.torso) != 1:
            if self.toon.style.getGender() == 'm' or ToonDNA.GirlBottoms[self.dna.botTex][1] == ToonDNA.SHORTS:
                torsoStyle = 's'
            else:
                torsoStyle = 'd'
            if self.dna.torso[1] != torsoStyle:
                self.dna.torso = self.dna.torso[0] + torsoStyle
                self.swapTorso(self.dna.torso)
    ...
    def swapTorso(self, type):
        torsoType = ToonDNA.toonTorsoTypes.index(type)
        torso = ToonDNA.toonTorsoTypes[torsoType]
        self.dna.torso = torso
        self.toon.swapToonTorso(torso)
        #self.toon.loop('run', 0)
    ...
    def enterNameShop(self):
        ...
        if self.progressing:
            self.hostTrack = Parallel(self.charRunFromLeft(self.host, newpos = Point3(4.9000000000000004, -1.8999999999999999, 0), newhpr = self.hostHpr, comm = self.ns.enter, eA = [
                ...
                self.nameList,
                self.warp]),
                self.charRunFromLeft(self.toon, newpos = Point3(-4.0999999999999996, -2.0, 0), newhpr = self.toonHpr))
            self.hostTrack.start()
