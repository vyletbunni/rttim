...
class BodyShop(StateData.StateData):
    ...
    def _BodyShop__swapTorso(self, offset):
        ...
        if not (self.clothesPicked):
            ...
            if ToonDNA.isValidBoyShirt(self.toon.style.topTex) == 0:
                pair = random.choice(ToonDNA.BoyShirts)
                self.toon.style.topTex = pair[0]
                self.toon.style.sleeveTex = pair[1]
            
            clength = len(ToonDNA.BoyShorts)
            ...
            if ToonDNA.isValidBoyShirt(self.toon.style.topTex) == 0:
                pair = random.choice(ToonDNA.GirlShirts)
                self.toon.style.topTex = pair[0]
                self.toon.style.sleeveTex = pair[1]
            
            clength = len(ToonDNA.GirlBottoms)
            if self.toon.style.botTex >= clength:
                if self.toon.style.torso[1] == 'd':
                    self.toon.style.botTex = ToonDNA.getRandomGirlBottom(ToonDNA.SKIRT)
                    torsoOffset = 3
                else:
                    self.toon.style.botTex = ToonDNA.getRandomGirlBottom(ToonDNA.SHORTS)
                    torsoOffset = 0
