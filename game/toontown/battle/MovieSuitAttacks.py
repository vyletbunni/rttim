...
def doSuitAttack(attack):
    ...
    elif name == VERYEVIL_EYE:
        suitTrack = doEvilEye(attack, 1)
    else:
...
def doDefault(attack):
    ...
    if suitName == 'f':
        ...
        attack['id'] = CIGAR_SMOKE
        attack['name'] = 'CigarSmoke'
        attack['animName'] = 'cigar-smoke'
        return doCigarSmoke(attack)
    elif suitName == 'mr':
        attack['id'] = EVIL_EYE
        attack['name'] = 'EvilEye'
        attack['animName'] = 'glower'
...
def getToonTrack(attack, damageDelay = 1e-06, damageAnimNames = None, dodgeDelay = 0.0001, dodgeAnimNames = None, splicedDamageAnims = None, splicedDodgeAnims = None, target = None, showDamageExtraTime = 0.01, showMissedExtraTime = 0.5, flexOnEx=0, prop=None):
    if not target:
        target = attack['target']
    toon = target['toon']
    battle = attack['battle']
...
def getToonTakeDamageTrack(toon, died, dmg, delay, damageAnimNames = None, splicedDamageAnims = None, showDamageExtraTime = 0.01, battle=None, prop=None):
    toonTrack = Sequence()
        ...
        for d in damageAnimNames:
            if toon.isDisguised:
                if d == 'conked':
                    ...
                    print battle
                    sinkPos1 = prop.getPos(battle)
                    sinkPos2 = prop.getPos(battle)
                    dropPos = prop.getPos(battle)
                    landPos = prop.getPos(battle)
                    sinkPos1.setZ(sinkPos1.getZ() - 3.1)
                    sinkPos2.setZ(sinkPos2.getZ() - 9.1)
                    dropPos.setZ(dropPos.getZ() + 15)
                    moveTrack = Sequence(Wait(0.9), LerpPosInterval(toon, 0.9, sinkPos1, other=battle), LerpPosInterval(toon, 0.4, sinkPos2, other=battle), Func(toon.setPos, battle, dropPos), Func(toon.wrtReparentTo, hidden), Wait(1.1), Func(toon.wrtReparentTo, battle), LerpPosInterval(toon, 0.3, landPos, other=battle))
...
def getSplicedAnimsTrack(anims, flexex = 0, actor = None):
    ...
    for nextAnim in anims:
        ...
        if len(nextAnim) <= 0:
            ...
            if nextAnim[0] == 'flail':
                pineapplepen = Parallel(ActorInterval(actor, nextAnim[0], playRate=1.52589),
                                        LerpPosInterval(actor, 2.416, (actor.getX(), actor.getY(), actor.getZ() - actor.height), blendType='easeIn'))
                track.append(pineapplepen)
            else:
...
def doEvilEye(attack, veryevil):
    ...
    if veryevil == 1:
        eye.setTexture(loader.loadTexture('phase_5/maps/veryevileye.jpg'), 1)
    damageDelay = 2.44
    dodgeDelay = 1.64
    suitName = suit.getStyleName()
    if suitName == 'cr':
        ...
        posPoints = [Point3(-0.64, 4.6, 7), VBase3(-155.0, -20.0, 0.0)]
    else:
        posPoints = [Point3(-0.4, 3.65, 5.01), VBase3(-155.0, -20.0, 0.0)]
    appearDelay = 0.8
    suitHoldStart = 1.06
...
def doSongAndDance(attack):
    ...
    damageAnims.append(['slip-backward',
     ...
     0.4,
     1.2])
    damageAnims.append(['slip-backward', 0.01, 1.0])
    dodgeAnims = [['victory']]
    
    suitsfx = base.loadSfx('phase_5/audio/sfx/SA_songanddance.ogg')
    throwSound = Sequence(SoundInterval(suitsfx, node=suit))
    hitsfx1 = base.loadSfx('phase_4/audio/sfx/avatar_emotion_surprise.ogg')
    hitsfx2 = base.loadSfx('phase_4/audio/sfx/MG_cannon_hit_dirt.ogg')
...
def doFlex(attack):
    ...
    damageAnims = [['cringe']]
    dodgeAnims = [['shrug']]
    suitsfx = base.loadSfx('phase_5/audio/sfx/SA_flex.ogg')
    ...
    throwSound = Sequence(Wait(0.6), SoundInterval(suitsfx, node=suit))
    toonsfx = base.loadSfx('phase_3.5/audio/sfx/avatar_emotion_shrug.ogg')
