...
def __createThrownTrapMultiTrack(trap, propList, propName, propPos = None, propHpr = None, anim = 0, explode = 0):
    ...
    if toon.isDisguised:
        if toon.suit.style.body == 'c':
            ...
            toonTrack = Sequence(Func(toon.headsUp, battle, targetPos), ActorInterval(toon.suit, 'throw-paper', playRate=1.2), Func(toon.suit.loop, 'neutral'))
    else:
...
def __createPlacedTrapMultiTrack(trap, prop, propName, propPos = None, propHpr = None, explode = 0, visibleOnlyForThisSuitId = None):
    ...
    if toon.isDisguised:
        toonTrack.append(Sequence(Wait(1), ActorInterval(toon.suit, 'speak', endFrame=29), ActorInterval(toon.suit, 'speak', startFrame=29, endFrame=0)))
    else:
