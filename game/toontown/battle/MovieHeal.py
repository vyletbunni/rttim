...
def __runToHealSpot(heal):
    ...
    if toon.isDisguised:
        runAnimI = ActorInterval(toon.suit, 'run', duration=runHealTime)
    else:
...
def __healTickle(heal, hasInteractivePropHealBonus):
    ...
    def scaleFeathers(feathers, toon = toon, target = target):
        ...
        toon.update(0)
        if toon.isDisguised:
            hand = toon.suit.getRightHands()[0]
        else:
    ...
    tHeal = 3.0
    if toon.isDisguised:
        manim = Sequence(Func(toon.suit.loop, 'neutral'), Wait(2), ActorInterval(toon.suit, 'pickpocket'))
    else:
        manim = ActorInterval(toon, 'tickle')
    if target.isDisguised:
        mtrack = Parallel(featherTrack, manim, __getSoundTrack(level, 1, node=toon),
                          Sequence(Wait(tHeal), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus),
                                   ActorInterval(target.suit, 'squirt-small-react'), Func(target.suit.loop, 'neutral')))
    else:
        mtrack = Parallel(featherTrack, manim, __getSoundTrack(level, 1, node=toon), Sequence(Wait(tHeal), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus), ActorInterval(target, 'cringe', startTime=20.0 / target.getFrameRate('cringe'))))
    track.append(mtrack)
