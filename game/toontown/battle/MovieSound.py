...
def __createToonInterval(sound, delay, toon, operaInstrument = None):
    ...
    if DISTANCE_TO_WALK_BACK and hasLuredSuits and not isNPC:
        if toon.isDisguised:
            retval.append(Parallel(ActorInterval(toon.suit, 'walk', startTime=1, duration=TIME_TO_WALK_BACK, endTime=0.0001), LerpPosInterval(toon, TIME_TO_WALK_BACK, newPos, other=battle)), Func(toon.suit.loop, 'neutral'))
        else:
    ...
    if operaInstrument:
        ...
        I1 = 2.8
        if toon.isDisguised:
            retval.append(Wait(1.6))
            retval.append(ActorInterval(toon.suit, 'glower', playRate=1.0, startTime=0.0, endTime=1.0))
            retval.append(Func(setPosFromOther, sprayEffect, operaInstrument, Point3(0, 1.6, -0.18)))
            retval.append(__getPartTrack(sprayEffect, 0.0, 6.0, [sprayEffect, toon.suit, 0], softStop=-3.5))
            retval.append(ActorInterval(toon.suit, 'glower', playRate=1.0, startTime=1.0))
        else:
        ...
        if toon.isDisguised:
            retval.append(Wait(1.6))
            retval.append(ActorInterval(toon.suit, 'glower'))
        else:
            retval.append(ActorInterval(toon, 'sound'))
    if DISTANCE_TO_WALK_BACK and hasLuredSuits and not isNPC:
        if toon.isDisguised:
            retval.append(Parallel(ActorInterval(toon.suit, 'walk', startTime=0.0001, duration=TIME_TO_WALK_BACK, endTime=1), LerpPosInterval(toon, TIME_TO_WALK_BACK, oldPos, other=battle)))
        else:
    ...
    if toon.isDisguised:
        retval.append(Func(toon.suit.loop, 'neutral'))
    else:
...
def __doBikehorn(sound, delay, toon, targets, level):
    ...
    def setInstrumentStats(instrument = instrument, instrument2 = instrument2, suitX=suitX, suitY=suitY, suitZ=suitZ, suitR=suitR):
        print 'M'
