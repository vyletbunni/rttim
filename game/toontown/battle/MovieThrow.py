...
def __throwPie(throw, delay, hitCount):
    ...
    if toon.isDisguised:
        if toon.suit.style.body == 'c':
            toonTrack.append(ActorInterval(toon.suit, 'throw-paper', playRate=0.9))
        else:
            toonTrack.append(ActorInterval(toon.suit, 'throw-paper', playRate=1.15))
    ...
    if toon.isDisguised:
        pieShow = Sequence(Func(MovieUtil.showProps, pies, hands), Func(pie2.setP, 180), Func(pie.hide))
    else:
    ...
    if toon.isDisguised:
        piePreflight = Func(__propPreflight, pies, suit, toon.suit, battle)
    else:
    ...
    if hitSuit:
        if toon.isDisguised:
            pieFly = Sequence(Func(pie.show),
                     LerpPosInterval(pie, tPieHitsSuit - tPieLeavesHand, pos=MovieUtil.avatarFacePoint(suit, other=battle), name=pieFlyTaskName, other=battle))
        else:
