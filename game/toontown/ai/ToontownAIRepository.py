...
from toontown.dna.DNAParserOld import loadDNAFileOldAI
from toontown.dna.DNAParserOld import loadDNAFile4ToonFest
...
class ToontownAIRepository(ToontownInternalRepository):
    ...
    def lookupDNAFileName(self, zoneId):
        ...
        if hood == 'toonfest' or hood == 'outdoor_zone':
    ...
    def loadDNAFileAI(self, dnastore, filename):
        print filename
        if filename[-4:] == '.dna':
            return loadDNAFileOldAI(dnastore, filename)
        else:
            return loadDNAFileAI(dnastore, filename)
    
    def loadDNA(self, filename):
        tree = loadDNAFile4ToonFest(filename)
