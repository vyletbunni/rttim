...
FO_DICT = {'s': 'tt_m_ara_cbe_fieldOfficeMoverShaker',
 ...
 'z': 'tt_m_ara_cbe_fieldOfficeMoverShaker'}
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def loadElevator(self, newNP, cogdo = False):
        ...
        if self.mode == 'suit':
            ...
            zIcon = loader.loadTexture('phase_3/maps/crazybot.jpg', 'phase_3/maps/crazybot_a.rgb')
            dept = chr(self.track)
            if dept == 'c':
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        print chr(self.track)
        if chr(self.track) == 'z':
            suitNP = loader.loadModel('phase_4/models/modules/suit_landmark_z')
            suitNP.setColor(1.0, 0.8, 0.8, 1.0)
        else:
